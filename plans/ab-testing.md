...
- Pause switches the flag off (decided 2026-09-26). `flagUpdateForStatusChange` maps status to flag: pausing sets `enabled: false`, starting or resuming sets `enabled: true`, and completing rolls out the winner. It's applied in the same transaction as the status change, with a version bump and cache invalidation. Pause asks for confirmation (it warns that anything else reading the flag also switches off), and a paused experiment shows "Resume". Verified with 6 fixed visitors: they split 3/3 while running, all got the "control" fallback while paused (no new exposures), and each got their original variant back on resume.
- Fixed a pre-existing tracking bug found along the way: a multivariate flag that assigns no variant (disabled, outside its rollout, or not targeted) evaluates to `false`. The script returned that `false` instead of the caller's fallback, recorded it as an exposure, and tagged every event with it, which put a bogus `false` arm in the results. The script now treats it as absent, and the results queries also ignore `'false'` for scripts still cached in browsers.
